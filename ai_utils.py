...
def classify_text_with_ai(text: str,
    ...
    cats_hash = hashlib.sha1(str(sorted(categories)).encode('utf-8')).hexdigest()
    ...
    # Передаём все категории (ранее было ограничение на 5)
    cat_subset = categories
...
Категории для классификации: "экскурсии", "клининнг", "услуги_мастеров", "бьюти", "аренда авто", "трансфер", "внж", "страховки", "недвижимость".
...
13. “🌊 Demre – Myra Tour 🌿 … Стоимость тура: 100€… Это идеальный тур!” → {{“relevant”: false, “category”: “экскурсии”, “subcategory”: null, “region”: “Демре”, “explanation”: “явное предложение тура”, “confidence”: 0.95}}"""
    ...
    user_prompt = f"Проанализируйте следующее сообщение и определите, является ли оно релевантным лидом. Ответьте ТОЛЬКО в формате JSON в одной строке:\n\n{text}"
    ...
    try:
        ...
        input_tokens = getattr(u, 'input_tokens', None) if hasattr(u, 'input_tokens') else (u.get('input_tokens') if isinstance(u, dict) else None)
        output_tokens = getattr(u, 'output_tokens', None) if hasattr(u, 'output_tokens') else (u.get('output_tokens') if isinstance(u, dict) else None)
        total_tokens = getattr(u, 'total_tokens', None) if hasattr(u, 'total_tokens') else (u.get('total_tokens') if isinstance(u, dict) else None)
        it_details = getattr(u, 'input_tokens_details', None) if hasattr(u, 'input_tokens_details') else (u.get('input_tokens_details') if isinstance(u, dict) else None)
        cached_tokens = None
        if it_details:
            cached_tokens = getattr(it_details, 'cached_tokens', None) if hasattr(it_details, 'cached_tokens') else (it_details.get('cached_tokens') if isinstance(it_details, dict) else None)
        resp_id = getattr(resp, 'id', None)
        ts = datetime.now().strftime("%m-%d %H:%M:%S")
        with open("ai_tokens.log", "a", encoding="utf-8") as tf:
...
def apply_overrides(cla, lower_text, category_heuristic):
    ...
    if "страховка" in lower:
        question_indicators = ["сколько стоит", "нужна", "хочу", "подскажите", "как получить", "какая", "можно ли", "ищу"]
        salesy = any(w in lower for w in ["купить", "оформить", "предлагаем", "продаем", "продаю", "выгодно", "предложение"])
    ...
    if (cla.get("category") == "недвижимость" or category_heuristic == "недвижимость"):
        seller_terms = [
            ...
            "цена снижена", "акция", "скидка", "без посредников", "риэлтор", "агентств", "риелтор", "посредник", "забронируй", "доступна"
        ]
        buyer_triggers = [
            "нужен", "нужна", "нужны",
            "ищу", "ищем",
